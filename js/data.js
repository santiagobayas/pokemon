...
// 12 POKÉMON SELECCIONABLES POR EL JUGADOR
const POKEMON_LIST = [
  {
    id: 'pikachu',
    name: 'Pikachu',
...
function generateRandomRivals(playerPokeId) {
  return RIVAL_SPAWN_LOCATIONS.map((loc) => {
    ...
    return {
      ...
      img: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${chosen.pokeId}.png`,
...
function preloadAllGameImages() {
  ...
  Object.values(RIVAL_TIERS).forEach(tierList => {
    tierList.forEach(r => {
      const primary = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${r.pokeId}.png`;
