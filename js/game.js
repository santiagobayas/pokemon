...
function renderCanvasWorld() {
  ...
  // Garantizar dimensiones fijas HD
  ...
  try {
    ...
    // --- TRASLACIÓN DE CÁMARA PARA EL MUNDO JUGABLE ---
    ctx.save();
    ctx.translate(-cameraX, 0);

    // Elementos de fondo temáticos del escenario
    drawSceneryBackgroundProps();

    // 5. Plataformas del mundo
    ...
    // 9. Partículas del mundo
    renderPlatformParticles(ctx);

    ctx.restore();
  } catch (err) {
    console.error('Error renderizando mundo:', err);
  }
}

function drawSceneryBackgroundProps() {
  // Zona 2: Árboles Robóticos de fondo
  const treeImg = loadedImages['assets/scenery/tree_bot.svg'];
  [920, 1380, 1720].forEach(tx => {
    if (treeImg && treeImg.complete && treeImg.naturalWidth > 0) {
      ctx.drawImage(treeImg, tx, 390, 80, 130);
    } else {
      // Dibujo de respaldo
      ctx.fillStyle = '#334155';
      ctx.fillRect(tx + 32, 440, 16, 80);
      ctx.fillStyle = '#10b981';
      ctx.beginPath();
      ctx.arc(tx + 40, 420, 36, 0, Math.PI * 2);
      ctx.fill();
    }
  });

  // Zona 3: Engranajes Gigantes Steampunk
  const gearBgImg = loadedImages['assets/scenery/gear_large.svg'];
  [2000, 2420, 2800].forEach((gx, idx) => {
    ctx.save();
    ctx.translate(gx, 340);
    ctx.rotate((animationTick * 0.02) * (idx % 2 === 0 ? 1 : -1));
    if (gearBgImg && gearBgImg.complete && gearBgImg.naturalWidth > 0) {
      ctx.drawImage(gearBgImg, -60, -60, 120, 120);
    } else {
  ...
  // Zona 4: Columnas de Energía
  const pillarImg = loadedImages['assets/scenery/tech_pillar.svg'];
  [3040, 3460].forEach(px => {
    if (pillarImg && pillarImg.complete && pillarImg.naturalWidth > 0) {
      ctx.drawImage(pillarImg, px, 360, 60, 160);
    } else {
      ctx.fillStyle = '#1e1b4b';
      ctx.fillRect(px, 360, 40, 160);
      ctx.fillStyle = '#a855f7';
      ctx.fillRect(px + 10, 370, 20, 140);
    }
  });
}
...
/* Helper universal para rectángulos redondeados (100% compatible con todos los navegadores) */
function drawSafeRoundRect(context, x, y, w, h, r) {
  let radius = { tl: 0, tr: 0, br: 0, bl: 0 };
  if (typeof r === 'number') {
    radius = { tl: r, tr: r, br: r, bl: r };
  } else if (Array.isArray(r)) {
    radius = { tl: r[0] || 0, tr: r[1] || 0, br: r[2] || 0, bl: r[3] || 0 };
  }
  context.beginPath();
...
function drawStylizedPlatform(p) {
  ...
  // 1. Tierra / Base estructural según zona del mapa
  const dirtGrad = ctx.createLinearGradient(p.x, p.y, p.x, p.y + p.h);
  if (p.x < 750) {
    // Zona 1: Tierra fértil natural
    dirtGrad.addColorStop(0, '#92400e');
    dirtGrad.addColorStop(1, '#451a03');
  } else if (p.x < 1800) {
    // Zona 2: Bosque Robótico (Corteza tecno-marrón)
    dirtGrad.addColorStop(0, '#334155');
    dirtGrad.addColorStop(1, '#0f172a');
  } else if (p.x < 2900) {
    // Zona 3: Acero Steampunk y bronce
    dirtGrad.addColorStop(0, '#78350f');
    dirtGrad.addColorStop(1, '#292524');
  } else {
    // Zona 4: Aleación cibernética de alta tecnología
    dirtGrad.addColorStop(0, '#1e1b4b');
    dirtGrad.addColorStop(1, '#0f172a');
  }

  ctx.fillStyle = dirtGrad;
  drawSafeRoundRect(ctx, p.x, p.y, p.w, p.h, radius);
  ctx.fill();

  // 2. Capa superior decorativa (Hierba, musgo LED, bronce o neón)
  const topHeight = p.isGround ? 16 : 10;
  const topGrad = ctx.createLinearGradient(p.x, p.y, p.x, p.y + topHeight);
  ...
  if (p.x < 750) {
    ...
    topGrad.addColorStop(0, '#facc15');
    topGrad.addColorStop(1, '#ca8a04');
  } else {
    topGrad.addColorStop(0, '#c084fc');
    topGrad.addColorStop(1, '#7e22ce');
  }

  ctx.fillStyle = topGrad;
  drawSafeRoundRect(ctx, p.x, p.y, p.w, topHeight, [radius, radius, 0, 0]);
  ctx.fill();

  // 3. Borde iluminado
  ctx.strokeStyle = p.x >= 2900 ? '#e9d5ff' : (p.x >= 1800 ? '#fef08a' : (p.x >= 750 ? '#bae6fd' : '#86efac'));
  ctx.lineWidth = 2;
  ctx.beginPath();
  ...
  // 4. Flores y detalles decorativos en plataformas aéreas
  if (!p.isGround) {
    if (p.x < 750) {
      drawFlower(p.x + 20, p.y - 4, '#f43f5e');
      drawFlower(p.x + p.w - 25, p.y - 4, '#38bdf8');
    } else if (p.x < 1800) {
      drawFlower(p.x + 20, p.y - 4, '#38bdf8');
      drawFlower(p.x + p.w - 25, p.y - 4, '#4ade80');
    } else if (p.x < 2900) {
      drawFlower(p.x + 20, p.y - 4, '#fb923c');
      drawFlower(p.x + p.w - 25, p.y - 4, '#facc15');
    } else {
      drawFlower(p.x + 20, p.y - 4, '#a855f7');
      drawFlower(p.x + p.w - 25, p.y - 4, '#38bdf8');
    }
  }

  // 5. Sombra de plataforma
  if (!p.isGround) {
...
function drawRival(r) {
  ...
  if (imgObj && imgObj.complete && imgObj.naturalWidth > 0) {
    ...
    // Avatar de Respaldo Garantizado si la imagen está cargando
    ctx.fillStyle = '#ef4444';
    ctx.beginPath();
    ctx.arc(r.x + r.w / 2, currY + r.h / 2, r.w * 0.45, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.font = 'bold 20px Fredoka One, Nunito';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(r.name.charAt(0), r.x + r.w / 2, currY + r.h / 2);
    ctx.textBaseline = 'alphabetic';
  }

  // Cartelito con el nombre del rival usando drawSafeRoundRect
  ctx.fillStyle = 'rgba(15, 23, 42, 0.88)';
  drawSafeRoundRect(ctx, r.x + r.w / 2 - 40, currY - 26, 80, 20, 10);
  ctx.fill();
...
function drawPlayer() {
  ...
  if (pImg && pImg.complete && pImg.naturalWidth > 0) {
    ...
    if (player.facing === 'right') {
      ...
      ctx.drawImage(pImg, drawX, drawY, player.w, player.h);
    }
  } else {
    // Avatar de Respaldo Garantizado si la imagen está cargando
    ctx.fillStyle = '#3b82f6';
    ctx.beginPath();
    ctx.arc(drawX + player.w / 2, drawY + player.h / 2, player.w * 0.45, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(selectedPoke.name.charAt(0), drawX + player.w / 2, drawY + player.h / 2);
    ctx.textBaseline = 'alphabetic';
  }
